    ...
    <section ref={revealRef} className="relative bg-[#061E31] py-24 lg:py-32 overflow-hidden">
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        >
          ...
          <p className="mt-5 text-[16px] text-white/55 max-w-xl leading-[1.6]">{t.benchmark.subtitle}</p>
        </div>

        <div className="grid grid-cols-12 gap-10">
          {/* Left — copy */}
          <div
            className={`col-span-12 lg:col-span-7 transition-all duration-700 delay-100 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          >
            ...
            {/* Terminal */}
            <div className="mt-10 inline-block px-4 py-2.5 rounded-md bg-black/40 border border-white/10 font-mono text-[13px] text-[#FFB814] min-w-[260px]" dir="ltr">
          ...
          {/* Right — metric cards */}
          <div
            className={`col-span-12 lg:col-span-5 transition-all duration-700 delay-200 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          >
            <div className="text-[11px] font-bold tracking-[0.16em] text-[#FFB814]/70 mb-5 uppercase">{t.benchmark.measures}</div>
            <div className="space-y-2.5">
              {t.benchmark.metrics.map((m, i) => {
                const cfg = metricConfig[i];
                return (
                  <MetricRow key={i} m={m} cfg={cfg} idx={i} visible={visible} />
                );
              })}
            </div>
          </div>
...
const metricConfig = [
  { pct: 85,   value: '↓72%',   color: '#1173BD', label: 'MTTR' },
  { pct: 99.9, value: '99.9%',  color: '#22C55E', label: 'SLA'  },
  { pct: 94,   value: '94%',    color: '#1173BD', label: 'FCR'  },
  { pct: 96,   value: '96%',    color: '#FFB814', label: 'CSAT' },
  { pct: 99.9, value: '99.9%',  color: '#22C55E', label: 'UP'   },
];

const MetricRow = ({ m, cfg, idx, visible }) => (
  <div
    className={`group px-5 py-4 rounded-xl border transition-all cursor-default ${
      visible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'
    }`}
    style={{
      transitionDelay: `${idx * 80 + 300}ms`,
      transition: 'opacity 0.5s, transform 0.5s, border-color 200ms ease, background-color 200ms ease',
      background: 'rgba(255,255,255,0.04)',
      borderColor: 'rgba(255,255,255,0.08)',
    }}
    onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.07)'; e.currentTarget.style.borderColor = `${cfg.color}40`; }}
    onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; }}
  >
    <div className="flex items-center gap-3 mb-3">
      <span className="text-[11px] font-black tracking-widest text-white/40">{cfg.label}</span>
      <span className="text-[13px] text-white/70 flex-1">{m.label}</span>
      <span className="text-[14px] font-black tabular-nums" style={{ color: cfg.color }}>{cfg.value}</span>
    </div>
