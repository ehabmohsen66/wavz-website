...
function BoardForm(form, onChange) {
  return (
    <>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
        ...
        <Field label="Title (English)"><Input value={form.title_en} onChange={v=>onChange('title_en',v)} placeholder="Chairman" /></Field>
        <Field label="Title (Arabic)"><Input value={form.title_ar} onChange={v=>onChange('title_ar',v)} placeholder="رئيس مجلس الإدارة" /></Field>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
        ...
        <Field label="LinkedIn URL"><Input value={form.linkedin} onChange={v=>onChange('linkedin',v)} /></Field>
...
export default function Board() {
  return (
    <CrudPage
      ...
      subtitle="Manage board members displayed on the Board page"
      fetchFn={api.getBoard}
      columns={COLS}
      renderForm={BoardForm}
      createFn={api.createBoardMember}
      updateFn={api.updateBoardMember}
      deleteFn={api.deleteBoardMember}
      emptyForm={EMPTY}
    />
  );
}
