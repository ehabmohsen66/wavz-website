<?php
/**
 * WAVZ 1-Click Server-Side Extractor (from /api)
...
<?php
$zipPath = dirname(__DIR__) . '/cpanel-deploy.zip';
$targetDir = dirname(__DIR__);

if (!file_exists($zipPath)) {
    echo '<p class="error">❌ cpanel-deploy.zip was not found in ' . htmlspecialchars($targetDir) . '</p>';
    ...
    if ($res === true) {
        ...
        $zip->close();
        if ($extracted) {
            echo '<p class="success">✅ Successfully extracted ' . $numFiles . ' files into ' . htmlspecialchars($targetDir) . '!</p>';
            echo '<div class="log">';
            ...
            echo '<p class="error">❌ Failed to extract files. Check file permissions for ' . htmlspecialchars($targetDir) . '.</p>';
        }
    } else {
